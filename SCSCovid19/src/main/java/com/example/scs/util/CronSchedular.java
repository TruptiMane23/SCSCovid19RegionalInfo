		...
		try {
			...
			while (itr.hasNext()) {
				...
				if (!(row.getCell(0) == null)) {
					...
					int worldCount = worldDetailsDao.updateWorldCount(Id, totalCases, recoveredCases,
			...
			e.printStackTrace();
			System.out.println("worldCount Message .. " + e.getMessage());
		}
		logger.info("--------------Exit updateWorldCount-----------------");	
	}
